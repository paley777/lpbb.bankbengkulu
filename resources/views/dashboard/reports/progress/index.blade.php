                ...
                <div class="col">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb responsive-small">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Rekapitulasi Kemajuan Pembelajaran</li>
                        </ol>
                    </nav>
                    <div class="card text-bg-success bg-gradient mb-3 bg-opacity-100">
                        ...
                        <div class="card-body">
                            <h4 class=" responsive-p1 fw-semibold mb-3">Rekapitulasi Kemajuan Pembelajaran</h4>
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                ...
                                <table id="example" class="table table-hover responsive-small">
                                    ...
                                    @if ($progreses->count() == 0)
                                        <h4 class="fw-semibold responsive-p1 me-3">No Data</h4>
                                    @else
                                        <tbody>
                                            @foreach ($progreses as $key => $progress)
                                                <tr>
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td>{{ $progress->created_at->format('Y-m-d') }}</td>
                                                    <td>
                                                        @foreach ($users as $user)
                                                            @if ($progress->nrpp == $user->nrpp)
                                                                {{ $user->name }}
                                                            @endif
                                                        @endforeach
                                                    </td>
                                                    <td>{{ $progress->nrpp }}</td>
                                                    <td>{{ $progress->nama_modul }}</td>
                                                    <td>{{ $progress->nama_materi }}</td>
                                                    <td>{{ $progress->jenis }}</td>
